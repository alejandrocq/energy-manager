...
if __name__ == '__main__':
    ...
    while True:
        if (target_date is None or target_date.date() != datetime.now().date()) or target_hour_first_period is None:
            ...
            print(f"Loading prices data for {target_date.date()}")
            ...
            if response.status_code == 200:
                ...
                target_hour_first_period = min([(hour, price) for hour, price in hourly_prices if 3 <= hour <= 10],
                                               key=lambda x: x[1])
                target_hour_second_period = min([(hour, price) for hour, price in hourly_prices if 14 <= hour <= 23],
                                                key=lambda x: x[1])
                ...
                print(
                    f"Successfully downloaded prices data for {target_date.date()}. Email with all data has been sent.")
            else:
                print("Failed to download prices data")
        else:
            ...
            if is_first_target or is_second_target:
                if not plug.get_status():
                    ...
                    print(f"Plug enabled at {datetime.now()} for {delay} seconds")
                    send_email("Plug enabled", f"🔌 Plug has been enabled for {delay} seconds.",
                               manager_from_email, manager_to_email)
